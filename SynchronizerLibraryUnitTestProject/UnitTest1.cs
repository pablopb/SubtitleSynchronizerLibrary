...
using SubtitleSynchronizerLibrary;
using System.IO;
using System.Reflection;

namespace SynchronizerLibraryUnitTestProject
{
    public class Tests
    {
        private readonly string subtitleFilePath = "C:\\tmp\\sample.srt";
        private readonly string newSubTitleFilePath = "C:\\tmp\\newSample.srt";
        private readonly string sampleFileText = @"1
...
me a higher interest rate.";
        ...
        public void Setup()
        {
           using(var stream = new StreamWriter(subtitleFilePath))
            {
                stream.Write(sampleFileText);
            }
        }
        ...
        [Test]
         public void TestFileReadOperation()
         {
             ...
             var synchronizer = new SubtitleSynchronizer();
             var result = synchronizer.ReadSubTitleFile(subtitleFilePath);
             Assert.AreEqual(ResultStatus.SUCCESS,result.Result);
         }

        [Test]
        public void TestOffSetOperationInSeconds()
        {

            var synchronizer = new SubtitleSynchronizer();
            synchronizer.ReadSubTitleFile(subtitleFilePath);
            var result = synchronizer.ApplyOffsetToTheSubtitleInSeconds(-1);
            Assert.AreEqual(ResultStatus.SUCCESS, result.Result);
        }

        [Test]
        public void TestOffSetOperationInMilliseconds()
        {

            var synchronizer = new SubtitleSynchronizer();
            synchronizer.ReadSubTitleFile(subtitleFilePath);
            var result = synchronizer.ApplyOffsetToTheSubtitleInMilliseconds(-100);
            Assert.AreEqual(ResultStatus.SUCCESS, result.Result);
        }

        [Test]
        public void TestSaveFileOperation()
        {

            var synchronizer = new SubtitleSynchronizer();
            synchronizer.ReadSubTitleFile(subtitleFilePath);
            synchronizer.ApplyOffsetToTheSubtitleInMilliseconds(-100);
            var result = synchronizer.GenerateSubtitleFileWithTheNewOffSet(newSubTitleFilePath);
            Assert.AreEqual(ResultStatus.SUCCESS, result.Result);
        }
    }
}